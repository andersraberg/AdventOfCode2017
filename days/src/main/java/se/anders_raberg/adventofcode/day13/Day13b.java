...
public class Day13b {
	private static final Logger LOGGER = Logger.getLogger(Day13b.class.getName());
	...
	public static void run(String inputPath) throws IOException {
		...
		int d = 0;
		while(true) {
			layers.values().forEach(Layer::resetScanner);			
			int delay = d;
			layers.values().forEach(l -> l.moveScanner(delay));
